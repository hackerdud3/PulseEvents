...
const EveCard = (props: Props) => {
  ...
  const toggleInterest = () => {
    ...
    setLiked(newLiked);
    setEvent({ ...event, attending: Array.from(newLiked) });
    updateEvent(event);
  };
  ...
  return (
    <Suspense fallback={<Cardloading />}>
      <div className="sm:w-[362px] w-[200px] inline-block">
        <Disclosure>
          {({ open }) => (
            <>
              <Disclosure.Button className="flex flex-col w-full rounded-lg bg-white shadow-sm text-left text-sm font-medium hover:bg-gray-100 focus:outline-none focus-visible:ring focus-visible:ring-purple-500 focus-visible:ring-opacity-75">
                <div className="sm:h-36 h-24 w-full ">
                  <Image
                    width={1000}
                    height={1000}
                    src={imageData}
                    alt="event-image"
                    className="w-full sm:h-36 h-24 rounded-tl-lg rounded-tr-lg overflow-hidden object-cover object-bottom "
                  />
                </div>

                <div className="p-4 sm:h-full w-full justify-between flex flex-col ">
